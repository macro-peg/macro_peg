...
def galil(m):
    ...
    while m.avail < n:
        ...
        if h is None and pending is not None:
            ...
            if verify is not None:
                ...
                if pending is not None and verify > R:
                    ...
                    Dc = D; Dlo = C - h                  # cyclic walker over [C-h, C]
                    lag = 0; off = 0                     # L is real
                    m.tick(h); yield
        # --- extension ---------------------------------------------------------
        if h is not None and off > 0:
            left = m.rd(R - lag)                     # mirror read while L is virtual
            assert L >= 3 and left == m.rd(L - 2), ('mirror', L, R, lag, off)
        else:
            left = m.rd(L - 2) if L >= 3 else None
        if left == c:
            yield
            L -= 2; R += 2
            if h is not None:
        ...
        if h is not None and m.rd(Dc) == c:
            assert m.rd(D) == c
            yield
            C += h // 2; L += h - 2; R += 2; D = L - 2 + h
            Dc -= 2
            if Dc < Dlo: Dc += h
            assert Dc == D or (Dc - D) % h == 0, ('Dc after move', Dc, D, h)
            off += h - 2; lag = lag + h if lag > 0 else h - 2
            m.tick(h); yield
            m.out.append(1 if L == 1 else 0)
            continue
        ...
        if newL is None:
            ...
            yield
        R += 2; L = newL; C = (L + R) // 2
        h = None; D = None; pending = None; verify = None; lag = 0; off = 0
