...
Current prototype coverage includes `class`/`module`/`def` (including class superclass headers like `class C < Base`, singleton class `class << self`, and punctuated method names like `empty?`), arrays/hashes, symbols, `if/elsif/else`, `unless`, `for ... in ... end`, `begin/rescue/ensure` (+ `retry`), postfix modifiers (`stmt if cond` / `stmt unless cond`), `return`, `self`, instance/class/global variables (`@x`, `@@x`, `$x`), constant-path references (`A::B`), single/percent-quoted string literals (`'x'`, `%q{...}`, `%{...}`), command-style no-parentheses calls (`puts :ok`, `add 1, 2`), dot-call chains (including no-arg links like `user.profile.name`), bracket index calls (`ENV["HOME"]`), range expressions (`1..5` / `1...5`), comparison/logical/unary operators (`==`, `!=`, `<`, `>`, `&&`, `||`, `!`, `and`, `or`), block-pass params/args (`&block`), call-attached blocks (`do/end`, `{}`), and newline-separated statements.
