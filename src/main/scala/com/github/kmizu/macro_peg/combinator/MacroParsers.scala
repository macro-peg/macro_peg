 ...
 *
  ...
  final case class OptionParser[T](parser: MacroParser[T]) extends MacroParser[Option[T]] {
    override def apply(input: String): ParseResult[Option[T]] = {
      parser(input) match {
  ...
  final case class RepeatParser[T](parser: MacroParser[T]) extends MacroParser[List[T]] {
    override def apply(input: String): ParseResult[List[T]] = {
      var rest = input
  ...
  final case class Alternation[T, U >: T](a: MacroParser[T], b: MacroParser[U]) extends MacroParser[U] {
    override def apply(input: String): ParseResult[U] = a(input) match {
  ...
  final case class Sequence[T, U](a: MacroParser[T], b: MacroParser[U]) extends MacroParser[(T, U)] {
    override def apply(input: String): ParseResult[(T, U)] = a(input) match {
  ...
  final case class AndParser[T](p: MacroParser[T]) extends MacroParser[Any] {
    override def apply(input: String): ParseResult[Any] = p(input) match {
  ...
  final case class RewriteableParser[T](var p: MacroParser[T]) extends MacroParser[T] {
    override def apply(input: String): ParseResult[T] = p(input)
  }
}
